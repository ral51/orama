...
export const getMaxTextWidth = (theme, ticks) => {
  ...
  const maxWidth = _.reduce(
    ticks,
    (acc, d) => _.max([acc, ctx.measureText(d.text).width]),
    0
  );
  ctx.restore();
  return maxWidth;
...
const getLeftMargin = props => {
  ...
  const yTickCount =
    props.yTickCount || getTickCount({ ...props, yRange }, "y");
...
export const getPlotRect = props => {
  ...
  let partialPlotRect = rectUtils.marginInset(
    { bottom, top },
    { width, height }
  );
  const left = getLeftMargin({ ...props, plotRect: partialPlotRect });
  partialPlotRect = rectUtils.marginInset(
    { bottom, top, left },
    { width, height }
  );
