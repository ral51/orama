...
export const fitCheckText = (textObj, canvasWidth, canvasHeight, theme) => {
  const updateTextObj = (key, toAdd) =>
    update(textObj, key, val => val + toAdd);
...
const getTextBounds = (textObj, isRotated, theme) => {
  ...
  const textWidth =
    getTextWidth(theme, textObj.text) * (textAlign === "center" ? 0.5 : 1);
  ...
  return {
    textStart: !(isRotated ^ leftAligned)
      ? initialCoord - textWidth
      : initialCoord,
    textEnd: !(isRotated ^ rightAligned)
      ? initialCoord + textWidth
      : initialCoord
  };
};
